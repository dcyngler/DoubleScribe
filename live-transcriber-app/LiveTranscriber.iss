; Inno Setup script for Live Transcriber
; Builds a per-user installer (no admin required) from the PyInstaller onedir output.
;
;   Compile: "%LOCALAPPDATA%\Programs\Inno Setup 6\ISCC.exe" LiveTranscriber.iss
;   Output:  installer\LiveTranscriberSetup.exe
;
...
; %LOCALAPPDATA%\LiveTranscriber, so it survives uninstall/reinstall and is never placed in
...
DefaultDirName={autopf}\LiveTranscriber
...
Source: "dist\LiveTranscriber\*"; DestDir: "{app}"; Flags: recursesubdirs createallsubdirs ignoreversion
